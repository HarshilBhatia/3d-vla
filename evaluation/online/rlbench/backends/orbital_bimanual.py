...
from .bimanual import (  # noqa: F401
    Actioner,
    Mover,
...
class RLBenchEnv(BimanualRLBenchEnv):
    ...
    def __init__(
        ...
        if (calibration_registry is None) != (calibration_id is None):
            raise ValueError("calibration_registry and calibration_id must be supplied together")
        if calibration_registry is not None and any(
            ...
            raise ValueError(
                "A materialized calibration realization cannot be combined with "
                "legacy calibration construction arguments"
            )
        ...
        if miscal_camera_indices is None:
            self._miscal_camera_indices = None
        else:
            self._miscal_camera_indices = tuple(sorted({int(i) for i in miscal_camera_indices}))
            invalid = set(self._miscal_camera_indices) - set(range(len(apply_cameras)))
            if invalid:
                raise ValueError(f"miscal_camera_indices out of range: {sorted(invalid)}")
        ...
        # Canonical evaluation route: use a complete, named delta directly.
        # No random sampling or component composition occurs at rollout time.
        if calibration_registry is not None:
            self._miscal_T = calibration_transform_table(
                calibration_registry, calibration_id, tuple(apply_cameras)
            )
            print(
                f"[orbital bimanual eval] calibration realization: id={calibration_id!r}, "
                f"registry={calibration_registry}, cameras={tuple(apply_cameras)}",
                flush=True,
            )
            return

        # Legacy compatibility route. Fixed per-group base, then random top-up.
        # Composition order matches training: T_applied = T_random @ T_base.
        T_base = None
        ...
        if self._miscal_T is not None and self._miscal_camera_indices is not None:
            mask = torch.zeros(len(apply_cameras), dtype=torch.bool)
            mask[list(self._miscal_camera_indices)] = True
            identity = torch.eye(4, dtype=self._miscal_T.dtype).expand_as(self._miscal_T)
            self._miscal_T = torch.where(mask[:, None, None], self._miscal_T, identity)
            print(
                f"[orbital bimanual eval] applying miscalibration only to camera "
                f"indices {self._miscal_camera_indices} in order {tuple(apply_cameras)}",
                flush=True,
            )
