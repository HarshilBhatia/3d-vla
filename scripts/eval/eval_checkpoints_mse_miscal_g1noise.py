"""Evaluate checkpoints with per-task miscal noise forced to G1 camera group.

Identical to eval_checkpoints_mse_miscal.py except that camera_group is overridden
to noise_group_override (default 1) before the noise lookup, so the per-task key
becomes "{task}_G1" regardless of what camera_group is stored in the zarr.

Usage:
    python scripts/eval/eval_checkpoints_mse_miscal_g1noise.py \
        checkpoints=train_logs/Orbital/deltaM_miscal_per_task_med_new/interm_step_10000.pth \
        data_paths=/grogu/user/harshilb/G3_val_set/val.zarr \
        val_instructions=instructions/peract/instructions.json \
        bimanual=false \
        orbital_miscal_noise_level_per_task_group=medium \
        output_csv=results/checkpoint_mse_miscal/step_10000_g1noise_medium.csv \
        num_batches=100
"""
import csv
import re
...
def eval_one_datapath(model, tokenizer, preprocessor, loader, num_batches, amp_dtype,
                      relative_action, noise_group_override):
    accum = {}
    seen = 0
    ...
    for i, sample in tqdm(enumerate(loader), total=num_batches, desc="  batches"):
        ...
        # Override camera_group so the per-task noise lookup uses the specified group
        # regardless of what group is stored in the zarr.
        if sample.get("camera_group") is not None:
            sample["camera_group"] = torch.full_like(sample["camera_group"], noise_group_override)
...
def write_csv_rows(output_csv, ckpt_step, miscal_level, ckpt_results):
...
def print_results(ckpt_path, ckpt_step, miscal_level, ckpt_results):
    print(f"\n{'=' * 80}")
    print(f"Checkpoint: {ckpt_path}  (step {ckpt_step})  miscal={miscal_level}")
    print("-" * 80)
    for data_path, (metrics, n) in ckpt_results.items():
...
def main():
    ...
    noise_group_override = int(custom.get("noise_group_override", 1))
    ...
    noise_label = miscal_level_per_task or miscal_level or "none"
    csv_miscal_label = f"{noise_label}_G{noise_group_override}noise"
    ...
    for p in data_paths:
        print(f"  {p}")
    print(f"miscal_level={csv_miscal_label}  noise_group_override=G{noise_group_override}  num_batches={num_batches}  output_csv={output_csv}")
    print("-" * 80)
    ...
    for ckpt_path in checkpoints:
        ...
        print_results(ckpt_path, ckpt_step, csv_miscal_label, ckpt_results)
        write_csv_rows(output_csv, ckpt_step, csv_miscal_label, ckpt_results)
