...
def write_csv_rows(output_csv, ckpt_step, cfg_scale, miscal_level, ckpt_results):
    ...
    with open(output_csv, "a", newline="") as f:
        ...
        for data_path, (metrics, n) in ckpt_results.items():
            row = {
                ...
                "miscal_level": miscal_level or "none",
...
def print_results(ckpt_path, ckpt_step, cfg_scale, miscal_level, ckpt_results):
    print(f"\n{'=' * 80}")
    print(
        f"Checkpoint: {ckpt_path}  (step {ckpt_step})  "
        f"cfg_scale={_cfg_label(cfg_scale)}  miscal={miscal_level or 'none'}"
    )
    print("-" * 80)
    for data_path, (metrics, n) in ckpt_results.items():
...
def main():
    ...
    miscal_level = _parse_miscal_level(custom.get("orbital_miscal_noise_level"))
    miscal_level_per_task = _parse_miscal_level(
    ...
    for p in data_paths:
        print(f"  {p}")
    print(
        f"cfg_scale={_cfg_label(cfg_scale)}  miscal_level={csv_miscal_label}  "
    ...
    for ckpt_path in checkpoints:
        ...
        print_results(ckpt_path, ckpt_step, cfg_scale, csv_miscal_label, ckpt_results)
        write_csv_rows(output_csv, ckpt_step, cfg_scale, csv_miscal_label, ckpt_results)
