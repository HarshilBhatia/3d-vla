"""Plot MSE metrics vs training step, broken down by drawer variation (bottom/middle/top).

Results directory layout expected:
    <results_dir>/cfg_<N>/step_<S>.csv

Each CSV has columns:
    train_step, cfg_scale, dataset, variation, variation_name, n_samples,
...
each showing 3 lines (bottom / middle / top) vs training step.

Usage:
    python scripts/eval/plot_checkpoint_mse_cfg_variation.py results/variation/siglip_single_group/
    python scripts/eval/plot_checkpoint_mse_cfg_variation.py results/variation/siglip_multi_group/ --out plots/variation/mg/
"""
import argparse
import re
...
VARIATION_COLORS = {0: "#1f77b4", 1: "#ff7f0e", 2: "#2ca02c"}
...
def load_all(results_dir: Path) -> pd.DataFrame:
    ...
    for f in csvs:
        df = pd.read_csv(f)
        # train_step is in the CSV; also parse from filename as fallback
    ...
    df = df.sort_values(["cfg_scale", "train_step", "variation"]).reset_index(drop=True)
    ...
    print(f"  Variations:  {sorted(df['variation'].unique())}  (0=bottom 1=middle 2=top)")
...
def plot_metric_dataset(df_ds, dataset, metric, cfg_scales, out_dir: Path):
    ...
    for ax, cfg in zip(axes, cfg_scales):
        ...
        for var_id, var_name in VARIATION_LABELS.items():
            subset = df_cfg[df_cfg["variation"] == var_id].sort_values("train_step")
            if subset.empty:
                continue
            ax.plot(
                subset["train_step"],
