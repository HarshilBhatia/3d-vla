...
files=(
   train_logs/exp/main_camtoken_deltaM_full/best.pth
   train_logs/exp/main_camtoken_deltaM_full_full/best.pth
   train_logs/exp/main_camtoken_RT_full/best.pth
)

for file in "${files[@]}"; do
    ssh harshil@18.232.187.136 "mkdir -p $(dirname "$file")"
    rsync -avz "$file" harshil@18.232.187.136:"$file"
done